...
// === Website Un-Framing Proxy Route ===
// Allows embedding websites like StreamElements, Kick, TikTok, YouTube, Twitch, etc., stripping X-Frame-Options & CSP headers
app.get('/api/proxy-view', async (req, res) => {
    ...
    try {
        ...
        console.error(`[Proxy View Error] for ${targetUrl}:`, err.message);
        res.removeHeader('X-Frame-Options');
        res.removeHeader('Content-Security-Policy');
        res.setHeader('Content-Type', 'text/html; charset=utf-8');
        res.send(`
            ...
            <head>
                <meta charset="UTF-8">
                <style>
                    body { background: #12141d; color: #e3e5eb; font-family: 'Segoe UI', Arial, sans-serif; display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100vh; margin: 0; padding: 20px; text-align: center; box-sizing: border-box; }
                    .btn { background: linear-gradient(90deg, #58a6ff, #9146FF); color: #fff; text-decoration: none; font-weight: bold; padding: 12px 24px; border-radius: 8px; font-size: 14px; margin-top: 16px; display: inline-block; box-shadow: 0 4px 14px rgba(88,166,255,0.4); }
                </style>
            </head>
            <body>
                <span style="font-size: 3em; margin-bottom: 10px;">🌐</span>
                <h3 style="margin: 0 0 8px 0;">Website Restricted</h3>
                <p style="color: #8c92a6; max-width: 400px; margin: 0 0 16px 0; font-size: 0.9em;">
                    This site restricts direct iframe embedding. Tap below to open it in a browser window.
                </p>
                <a class="btn" href="${targetUrl}" target="_blank">↗ Open Website in Browser</a>
            </body>
            </html>
        `);
    }
